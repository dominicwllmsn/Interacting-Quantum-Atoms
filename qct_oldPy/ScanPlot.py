...
for run in range (thetaIn,thetaFin+1, thetaStep):
    ...
    for i in range(0, len(separations), 1):
        ...
        try:
            ...
            print("Path:", path, ", Scan", i, " --  f1.int does not exist")
            try:
                del separations[i]
            except IndexError:
                continue
        ...
        try:
            ...
            print("Path:", path, ", Scan", i, " --  f3.int does not exist")
            try:
                del separations[i]
            except IndexError:
                continue
    ...
    print(defEnergies)
    print(len(defEnergies))
    #Now plot the deformation energies against the separations.
    #pyplot.plt.plot(separation, defEnergies);
    #plt.xlabel('Separation (A)');
    #plt.ylabel('Deformation energy (Hartrees)');
    #pyplot.plt.show();
    
    #Create a text file with the raw data.
    dataFile = open(cwd + "/rawdata" + str(run) + ".txt", 'w+');
    for i in range(0, len(separations), 1):
      try:
